...
func _update_world_animation(delta: float) -> void:
	...
	for i in range(animated_flags.size()):
		...
		if not is_instance_valid(flag):
			continue
		flag.rotation_degrees.z = sin(animation_time * 2.2 + i * 0.9) * 5.0

	for i in range(animated_flames.size()):
		var flame := animated_flames[i]
		if not is_instance_valid(flame):
			continue
		var pulse := 1.0 + sin(animation_time * 7.0 + i) * 0.12
		flame.scale = Vector3(0.9 + pulse * 0.1, pulse, 0.9 + pulse * 0.1)
		if i < animated_torch_lights.size():
			var torch_light := animated_torch_lights[i]
			if is_instance_valid(torch_light):
				torch_light.light_energy = 1.0 + sin(animation_time * 6.0 + i) * 0.18
	...
	for i in range(animated_fountain_water.size()):
		...
		if not is_instance_valid(water):
			continue
		water.scale = Vector3(
			1.0 + sin(animation_time * 2.4 + i) * 0.025,
			1.0 + sin(animation_time * 3.0 + i) * 0.08,
			1.0 + sin(animation_time * 2.4 + i) * 0.025
		)
		water.rotation_degrees.y = fmod(animation_time * 7.0 + i * 45.0, 360.0)

	for i in range(animated_flowers.size()):
		var flower := animated_flowers[i]
		if not is_instance_valid(flower):
			continue
		flower.rotation_degrees.z = sin(animation_time * 1.8 + i * 0.7) * 2.0
...
func _create_flag(pos: Vector3, color: Color) -> void:
	...
	banner.position = Vector3(0.8, 1.0, 0)
...
func _create_torch(pos: Vector3) -> void:
	...
	add_child(light)
	animated_torch_lights.append(light)
...
# ============================================================
# DETECTAR / INTERACTUAR / DIÁLOGOS / CRIATURAS
# ============================================================
